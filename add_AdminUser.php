...
{	
 $sql= "INSERT INTO `tbl_admin`(`name`,`admin_name`,`password`,`role`,`expiry_date`,`status`,`date`) VALUES ('". $name ."','". $user_name ."','". password_hash($repassword, PASSWORD_BCRYPT) ."','". $role ."','no','".$default_status."','".$time."')";
...
<head>
  ...
  <title>Control Panel | <?php if(@$_GET['id']!=''){echo "Edit";}else{echo "Add";} ?> Control PanelUser</title>
...
$sql_role="SELECT SQL_NO_CACHE * FROM `role` where `status`='1'";
  $result_role=mysqli_query($con,$sql_role)or die(mysqli_error($con));
